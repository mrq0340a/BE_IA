 ...
 */
public class Astar {
	private static final int INFINI = 1000; // the value of the vertex that has
											// an obstacle
	private Graph graph;
	private List<Vertex> order = new ArrayList<>(); // list of step
	private List<Pair> listCreate = new ArrayList<>();
	private List<Pair> listSeen = new ArrayList<>(); // List of developed vertex
														// during the routing
	...
	private void addfirt(Pair p) {
		...
		if (!p.getV().equals(start)) { // car on ferrait un detour inutile
			for (int i = 0; i <= listCreate.size(); i++) {
				if (i == listCreate.size()) {// cas ou on tous parcourus �
												// optimiser au cas ou
					listCreate.add(i, p);
					return;
				} else {
	...
	private List<Vertex> getVoisinInvFace(List<Vertex> listFace,
			List<Vertex> adj) {
	...
	private List<Vertex> getAdjacentFilter(Vertex v) {
		...
		if (!voisinFace.contains(v)) {
			// on est dans le cas ou le fis est ne fait pas face au pere
	...
	/**
	 * modify the list of order, insert the different step to attain the
	 * destination
	 * 
	 * @param v
	 *            : the goal
	...
	public void aStar(List<Vertex> orientation) {
		...
		do {
			...
			if (isGoal(p.getV())) {
				...
				putAdj(adjcent); // j'ajoute �a dans la liste des creer
			}
		} while (go);
	}
	...
	public static void main(String[] args) {
		...
		for (Vertex vertex : g.getVertex()) {
			...
			as.aStar(dir);
			System.out.println("de " + star.getNameV() + " a "
					+ vertex.getNameV());
			g.printAdjacent(as.getOrder());
	...
	private class Pair implements Comparable<Pair> {
		...
		public boolean equals(Object obj) {
			if (obj != null && obj.getClass() == this.getClass()) {
				...
				// return (this.v.getNameV().equals(p.v.getNameV()) && this.rate
				// == p.rate);
			}
			return false;
		}
